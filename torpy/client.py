...
class TorClient:
    ...
    @contextmanager
    def create_circuit(self, hops_count=3, exit_ip=None, fast_exit=True, guard_by_flags=None) -> 'ContextManager[TorCircuit]':
        with self.get_guard(guard_by_flags) as guard:
            yield guard.create_circuit(hops_count, exit_ip=exit_ip, fast_exit=fast_exit)
